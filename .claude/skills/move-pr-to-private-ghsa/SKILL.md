...
REPO=BoldGrid/boldgrid-backup        # owner/repo from the PR URL
PR=1313                             # public PR number
SLUG="${REPO#*/}"                   # → "boldgrid-backup"
...
SLUG="${REPO#*/}"                    # → "boldgrid-backup"
PFX="${KEY}-${SLUG}-${PR}"           # → "ENG7-XXXX-boldgrid-backup-1234"
```
...
// .cursor/working/${PFX}-advisory-payload.json
{
  ...
  "vulnerabilities": [
    {
      "package": { "ecosystem": "composer", "name": "boldgrid/boldgrid-backup" },
...
- **To genuinely clear an array, send a JSON body — not `-f 'field[]='`.** The form-encoded shape `--raw-field 'collaborating_users[]='` sends a literal empty string as a user login and the API rejects it with `422 — User '' not found, collaborating_users cannot be modified`. If you need to clear (or fully reset) a list, build a JSON file and pass `--input`:
  ```bash
  echo '{"collaborating_users": [], "collaborating_teams": ["boldgrid-backup-developers"]}' \
...
|---|---|---|
| `BoldGrid/boldgrid-backup` | `boldgrid-backup-developers` | https://github.com/orgs/BoldGrid/teams/boldgrid-backup-developers |

For `BoldGrid/boldgrid-backup` specifically, a complete advisory collaborator setup typically looks like:

```bash
gh api -X PATCH repos/BoldGrid/boldgrid-backup/security-advisories/$GHSA \
  -f 'collaborating_teams[]=boldgrid-backup-developers' \
...
**Reviewers:** must be added as advisory collaborators (`PATCH /repos/{REPO}/security-advisories/{GHSA}` with `collaborating_teams[]` and/or `collaborating_users[]`) before they can see the TPF or the private PR. Prefer the maintained team for the repo where one exists (e.g., `BoldGrid/boldgrid-backup-developers` for `BoldGrid/boldgrid-backup`); use `collaborating_users[]` only for individuals outside that team. Currently added: teams=`{TEAM_LIST}`, users=`{USER_LIST}`.
...
6. Add advisory collaborators (if reviewers aren't already):
                            ...
                            (prefer team over enumerating users where one exists;
                             ...
                             BoldGrid/boldgrid-backup the default team is
                             boldgrid-backup-developers.
