...
Adapted from the BoldGrid public-plugin security workflow (originally validated on a sibling product public PR → GHSA TPF move). Examples below use `BoldGrid/boldgrid-backup`. See "Caveats to surface every time" for the line-drift / event-state / attribution issues this skill exists to avoid re-encountering.
...
Per AGENTS.md "Working Files", every scratch file follows `{KEY}-{REPO}-{PR}-{role}.{ext}`. The "primary" PR for this skill is the TPF PR you're posting *into* — the source PR is referenced from the filename role, not the keying triple. `{REPO}` is the lowercase slug (right-hand half of `org/repo`); GHSA TPFs follow `boldgrid-backup-ghsa-…` (`ghsa` lowercase). Capture the prefix once in Phase 1 and reuse:

```bash
KEY=ENG7-2908                                # Jira ticket
TPF_REPO=BoldGrid/boldgrid-backup-ghsa-rgpr-5m2g-gvh2
TPF_PR=1                                     # TPF PR number (almost always 1)
TPF_SLUG="${TPF_REPO#*/}"                    # → "boldgrid-backup-ghsa-rgpr-5m2g-gvh2"
...
```bash
SRC_REPO=BoldGrid/boldgrid-backup
SRC_PR=1313
