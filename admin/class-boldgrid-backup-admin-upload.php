...
class Boldgrid_Backup_Admin_Upload {
	...
	public function verify_upload_access() {
		...
		if ( ! wp_verify_nonce( $nonce, 'upload_archive_file' ) ) {
			// Display an error notice.
			do_action(
				'boldgrid_backup_notice',
		...
		$uploading = isset( $_POST['uploading'] ) ?
			sanitize_text_field( wp_unslash( $_POST['uploading'] ) ) : '';
		if ( empty( $uploading ) ) {
			// Display an error notice.
			do_action(
				'boldgrid_backup_notice',
		...
		if (
			...
			! isset( $_FILES['file']['tmp_name'], $_FILES['file']['error'], $_FILES['file']['name'] )
		) {
			// Display an error notice.
			do_action(
				'boldgrid_backup_notice',
		...
		// The temporary path is generated by PHP and must remain unchanged for upload validation.
		$tmp_name  = (string) $_FILES['file']['tmp_name']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- PHP upload path validated with is_uploaded_file().
		$file_error = absint( $_FILES['file']['error'] );
		...
		if ( $file_error ) {
			...
			do_action(
				'boldgrid_backup_notice',
				Boldgrid_Backup_Admin_Utility::translate_upload_error( $file_error ),
				'notice notice-error is-dismissible'
			);
		...
		if ( ! preg_match( '/boldgrid-backup-.*-\d{8}-\d{6}/', $file_name ) ) {
			// Display an error notice.
			do_action(
				'boldgrid_backup_notice',
		...
		// Retain the validated values for the remaining upload methods.
		$_FILES['file']['name']  = $file_name;
		$_FILES['file']['error'] = $file_error;
	...
	public function check_filetype_ext() {
		// Validate input.
		if (
			empty( $_FILES['file'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
			! isset( $_FILES['file']['name'], $_FILES['file']['tmp_name'] ) // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
		) {
			return false;
		}

		// Get the upload file basename.
		$file_basename = basename( sanitize_file_name( wp_unslash( $_FILES['file']['name'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
		$tmp_name       = (string) $_FILES['file']['tmp_name']; // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- PHP upload path was validated with is_uploaded_file().

		// Validate the filename and mime type.
		$validate = wp_check_filetype_and_ext( $tmp_name, $file_basename );
	...
	public function handle_upload() {
		...
		$file = array(
			'name'     => isset( $_FILES['file']['name'] ) ? // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
				sanitize_file_name( wp_unslash( $_FILES['file']['name'] ) ) : '', // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
			'type'     => isset( $_FILES['file']['type'] ) ? // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
				sanitize_mime_type( wp_unslash( $_FILES['file']['type'] ) ) : '', // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
			'tmp_name' => isset( $_FILES['file']['tmp_name'] ) ? // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
				(string) $_FILES['file']['tmp_name'] : '', // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- PHP upload path was validated with is_uploaded_file().
			'error'    => isset( $_FILES['file']['error'] ) ? // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
				absint( $_FILES['file']['error'] ) : UPLOAD_ERR_NO_FILE, // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
			'size'     => isset( $_FILES['file']['size'] ) ? // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
				absint( $_FILES['file']['size'] ) : 0, // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by upload_archive_file() before this method is called.
		);
	...
	public function upload_archive_file() {
		...
		$file_name      = isset( $_FILES['file']['name'] ) ? // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by verify_upload_access() above.
			sanitize_file_name( wp_unslash( $_FILES['file']['name'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified by verify_upload_access() above.
		$file_save_path = $this->get_save_path( $file_name );
	...
	public function ajax_url_import() {
		...
		$url = ! empty( $_POST['url'] ) ? esc_url_raw( wp_unslash( $_POST['url'] ) ) : null;
	...
	public function archive_upload_action() {
		...
		if ( $page_is_bgbkup && ! empty( $_FILES['file'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Flag only; upload_archive_file() verifies the nonce before processing.
